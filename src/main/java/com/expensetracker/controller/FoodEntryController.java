...
public class FoodEntryController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> updateFoodEntry(@PathVariable Long id, @RequestBody Map<String, Object> body) {
        try {
            if (foodEntryService.getFoodEntryById(id).isEmpty()) {
                return ResponseEntity.notFound().build();
            }
            FoodEntry entry = new FoodEntry();
            entry.setAmount(((Number) body.get("amount")).doubleValue());
            ...
            FoodEntry updated = foodEntryService.updateFoodEntry(id, entry);
            if (updated != null) {
                return ResponseEntity.ok(updated);
            }
            return ResponseEntity.notFound().build();
        } catch (Exception e) {
            return ResponseEntity.status(500).body("Error updating food entry: " + e.getMessage());
        }
    }
}
